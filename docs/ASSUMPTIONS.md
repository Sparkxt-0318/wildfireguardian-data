...
| A-TER-5 | Aspect is **degrees clockwise from grid north (0–360)**, giving the **downslope-facing** direction, on **grid north, not true north**. The convergence is not corrected, but its magnitude is now computable rather than merely warned about: `crs.meridian_convergence_deg` gives it, and it is about +0.19° to +0.21° over the shipped Uljin study areas on EPSG:5187, reaching about ±1.1° across South Korea on the nationwide EPSG:5179. | `terrain.derivatives.aspect`; `crs.meridian_convergence_deg`; `FAILURE_MODES.md` F-TER-5. |
